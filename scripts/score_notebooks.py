...
def score_one(
    ...
    try:
        llm_result = grade_with_llm(client, model, reference_text, student_text, autograde, rubric)
    except Exception as exc:
...
def run_batch(
    ...
    logger.info("Model: %s  Base URL: %s", model, base_url)
    logger.info("Scoring %d submission(s) …", len(ir_files))

    for ir_path in ir_files:
        logger.info("[%s]", ir_path.name)
        try:
            score_one(ir_path, output_dir, client, model, reference_text, rubric)
        except Exception as exc:
